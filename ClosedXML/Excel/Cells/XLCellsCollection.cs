...
namespace ClosedXML.Excel
{
    ...
    {
        internal Dictionary<short, int> ColumnsUsed { get; } = new Dictionary<short, int>();
        ...
        public void Add(int row, short column, XLCell cell)
        {
            Count++;
        ...
        public void Remove(int row, short column)
        {
            Count--;
        ...
        {
            int finalRow = rowEnd > MaxRowUsed ? MaxRowUsed : rowEnd;
            short finalColumn = (short)(columnEnd > MaxColumnUsed ? MaxColumnUsed : columnEnd);
            for (int ro = rowStart; ro <= finalRow; ro++)
            {
                if (RowsCollection.TryGetValue(ro, out Dictionary<int, XLCell> columnsCollection))
                {
            ...
            Func<IXLCell, bool> predicate = null)
        {
            int finalRow = rowEnd > MaxRowUsed ? MaxRowUsed : rowEnd;
            short finalColumn = (short)(columnEnd > MaxColumnUsed ? MaxColumnUsed : columnEnd);
            short firstColumnUsed = finalColumn;
            var found = false;
            for (int ro = rowStart; ro <= finalRow; ro++)
            {
                if (RowsCollection.TryGetValue(ro, out Dictionary<int, XLCell> columnsCollection))
                {
                    for (var co = (short)columnStart; co <= firstColumnUsed; co++)
                    {
                        if (columnsCollection.TryGetValue(co, out XLCell cell)
                            && !cell.IsEmpty(options)
                            && (predicate == null || predicate(cell))
        ...
        public void RemoveAll(int rowStart, short columnStart,
                              int rowEnd, short columnEnd)
        {
            int finalRow = rowEnd > MaxRowUsed ? MaxRowUsed : rowEnd;
        ...
        public IEnumerable<XLSheetPoint> GetSheetPoints(int rowStart, short columnStart,
                                                        int rowEnd, short columnEnd)
        {
            int finalRow = rowEnd > MaxRowUsed ? MaxRowUsed : rowEnd;
            int finalColumn = columnEnd > MaxColumnUsed ? MaxColumnUsed : columnEnd;
        ...
        internal void SwapRanges(XLSheetRange sheetRange1, XLSheetRange sheetRange2, XLWorksheet worksheet)
        {
            ...
            short columnCount = (short)(sheetRange1.LastPoint.Column - sheetRange1.FirstPoint.Column + 1);
        ...
        {
            return RowsCollection.TryGetValue(row, out Dictionary<int, XLCell> columnsCollection)
